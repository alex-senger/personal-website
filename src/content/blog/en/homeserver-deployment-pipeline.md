...
Each image gets two tags: `latest`, and an immutable `sha-<commit>` tag. The second one matters more than it looks — it means every deployed state of the site maps back to an exact commit, and rolling back is just pointing the deployment at an older tag.
